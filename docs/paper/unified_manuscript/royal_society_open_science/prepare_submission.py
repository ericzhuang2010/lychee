#!/usr/bin/env python3
"""Build the Royal Society Open Science submission package."""
...
RSOS_ABSTRACT = (
    ...
    "major disease of lychee. Public RNA-sequencing cohorts offer a resource for "
    "identifying cultivar-dependent infection responses, but are readily "
    "over-interpreted when candidate selection and confirmation use the same data. "
    "We combined an exploratory reanalysis with a prospectively registered "
    "genome-wide discovery and external-evaluation workflow. A negative-binomial "
    "cultivar-by-infection analysis of 19,445 expressed genes yielded 262 statistical "
    "candidates; 206 passed mapping and gene-model quality control, 19 remained "
    "significant under an independent statistical framework and 16 passed all "
    "registered robustness checks. In a prespecified cross-context evaluation using "
    "an independent pericarp time course, two genes were supported and five showed "
    "significant opposite-direction effects; the internally robust and externally "
    "supported sets did not overlap beyond chance expectation. None of 18 exploratory "
    "candidates met the subsequent genome-wide interaction criterion. Deterministic "
    "evidence integration produced no highest-confidence candidates, retained 12 "
    "internally robust genes in a middle tier and retired 65 entities. These results "
    "show that prominent single-cohort candidates often fail formal interaction "
    "testing and cross-context evaluation. The openly archived workflow, complete "
    "results and explicit negative findings provide a reproducible baseline for "
...
FIGURES = [
    (
        1,
        "figure1_study_design_qc.png",
        "Figure_1_study_design_and_QC.png",
    ),
    (
        2,
        "figure2_discovery_legacy.png",
        "Figure_2_discovery_and_legacy_audit.png",
    ),
    (
        3,
        "figure3_robustness_external.png",
        "Figure_3_robustness_and_external_evaluation.png",
    ),
    (
        4,
        "figure4_pathways_signatures.png",
        "Figure_4_pathway_and_signature_evaluation.png",
    ),
    (
        5,
        "figure5_transcript_usage.png",
        "Figure_5_transcript_usage.png",
    ),
    (
        6,
        "figure6_orthogonal_tiers.png",
        "Figure_6_orthogonal_evidence_and_tiers.png",
    ),
]
...
def british_english(value: str) -> str:
    for american, british in BRITISH_SPELLINGS.items():
        value = re.sub(rf"\b{american}\b", british, value)
    return value
...
def add_title_page(document: Document) -> None:
    ...
    note.add_run("Article type: Research Article")
    ...
    for row_index, source_row in enumerate(rows):
        ...
        for column_index, cell in enumerate(row.cells):
            ...
            if column_index < len(source_row):
                add_inline(paragraph, british_english(source_row[column_index]))
            for run in paragraph.runs:
                run.font.name = "Times New Roman"
                run.font.size = Pt(8)
                if row_index == 0:
...
def add_main_body(document: Document, lines: list[str]) -> None:
    ...
    while index < end:
        ...
        if heading:
            ...
            add_inline(paragraph, british_english(heading.group(2)))
            index += 1
            continue
        ...
        while index < end:
            ...
            index += 1
        value = british_english(" ".join(paragraph_lines))
        paragraph = document.add_paragraph()
...
def add_declarations_and_data(document: Document) -> None:
    sections = [
        (
            "Ethics",
            "This study reanalysed publicly available sequencing datasets and involved "
        ...
        (
            ...
            "Eric Zhuang: conceptualization, data curation, formal analysis, investigation, "
            "methodology, project administration, software, validation, visualization, "
            "writing--original draft and writing--review and editing. The author approved "
            "the submitted version and is accountable for the work.",
        ),
        (
            "Competing interests",
        ...
        (
            "Accession numbers",
            "Sequence data analysed in this article can be found in the NCBI SRA and GEO "
            ...
            f"Supplementary data are permanently archived at {ZENODO_DOI}.",
        ),
        (
            "Use of artificial intelligence",
            "Generative AI (OpenAI Codex) was used to assist with language editing and "
            "journal-specific document formatting. It was not used to generate data, "
            "perform analyses, interpret results or draw scientific conclusions. The "
            "author reviewed and takes responsibility for all content.",
        ),
    ]
    ...
    document.add_paragraph("Supplementary material", style="Heading 1")
    intro = document.add_paragraph()
    add_inline(
        intro,
        f"The following items are archived in the electronic supplementary material at {ZENODO_DOI}:",
    )
...
def add_figure_legends(document: Document, legends: dict[int, str]) -> None:
    ...
    for number in range(1, 7):
        ...
        caption = british_english(legends[number])
...
def build_manuscript() -> Path:
    ...
    abstract = RSOS_ABSTRACT
    if len(re.findall(r"\b[\w'-]+\b", plain_markdown(abstract))) > 200:
    ...
    properties.subject = "Research Article submitted to Royal Society Open Science"
    properties.keywords = plain_markdown(KEYWORDS)
    ...
    output = OUTPUT_DIR / "RSOS_manuscript.docx"
...
def build_cover_letter() -> Path:
    ...
    properties.author = AUTHOR
    properties.subject = "Submission to Royal Society Open Science"
    ...
    paragraphs = [
        (
            ...
            "publication in Royal Society Open Science as a Research Article."
        ),
        (
        ...
        (
            "The work fits Royal Society Open Science's broad scientific scope and objective "
            "peer-review criterion. Its main contribution is a transparent candidate resource "
            "with explicit evidence boundaries, together with a practical demonstration that "
            "plausible single-cohort candidates may not survive a formal genome-wide "
            "interaction test. Its emphasis on rigorous inference, reproducibility and complete "
            "reporting should be useful to readers in plant biology, genomics and bioinformatics."
        ),
        (
            ...
            "study is a reanalysis of public sequencing data and involved no new experiments "
            "with humans, human tissue or animals. It received no external funding, and I "
    ...
    output = OUTPUT_DIR / "RSOS_cover_letter.docx"
    document.save(output)
    return output
...
""".encode()
        ...
        for member in archive.infolist():
            ...
            if any(marker in contents.lower() for marker in excluded_markers):
                ...
                contents = b"".join(
                    line
                    for line in lines
                    if not any(marker in line.lower() for marker in excluded_markers)
                )
    ...
    root = "RSOS_supplement_2026-09-05"
...
def copy_payload_files() -> list[Path]:
    ...
    for _, source_name, target_name in FIGURES:
        ...
        with Image.open(target) as image:
            ...
            if min(image.size) < 1000 or min(dpi) < 295:
                raise ValueError(
                    f"{target.name} does not meet the 300-DPI publication-file check: "
